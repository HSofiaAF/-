...
export const Navbar = ({ 
  ...
  const { currentUser, logout, switchDemoUser, isFirebaseActive } = useAuth();
              ...
              <div className="relative">
                ...
                {showUserMenu && (
                  <div className="absolute right-0 mt-2 w-56 rounded-2xl glass-card border border-slate-200/80 shadow-xl p-2 z-50 animate-in fade-in slide-in-from-top-2">
                    ...
                    {!isFirebaseActive && (
                      <div className="px-2 py-1.5 bg-amber-50 rounded-xl mb-1 text-[11px] text-amber-800">
                        <p className="font-bold flex items-center gap-1">
                          <Users className="w-3 h-3 text-amber-600" /> Cambiar de perfil (demo):
                        </p>
                        <div className="flex gap-1 mt-1.5">
                          <button
                            onClick={() => { switchDemoUser('papa'); setShowUserMenu(false); }}
                            className="flex-1 py-1 px-2 rounded-lg bg-white font-semibold text-slate-700 hover:bg-amber-100 transition shadow-2xs"
                          >
                            Papá (Alex)
                          </button>
                          <button
                            onClick={() => { switchDemoUser('mama'); setShowUserMenu(false); }}
                            className="flex-1 py-1 px-2 rounded-lg bg-white font-semibold text-slate-700 hover:bg-amber-100 transition shadow-2xs"
                          >
                            Mamá
                          </button>
