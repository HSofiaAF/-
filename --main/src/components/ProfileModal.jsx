...
export const ProfileModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2.5 sm:p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-3xl overflow-hidden shadow-2xl z-10 border border-slate-100 flex flex-col max-h-[90dvh] sm:max-h-[92vh]">
        
        {/* Header */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 sm:py-4 border-b border-slate-100 bg-gradient-to-r from-rose-50/70 via-pink-50/50 to-white shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-gradient-to-tr from-rose-500 to-pink-500 text-white flex items-center justify-center shadow-md shadow-rose-500/20">
              <Sparkles className="w-4 h-4" />
            </div>
            <div>
              <h3 className="text-sm sm:text-base font-black text-slate-900 font-heading">Mi Perfil Familiar</h3>
              <p className="text-[11px] sm:text-xs text-slate-500">Personaliza tu apodo, relación e icono</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSave} className="p-4 sm:p-6 space-y-4 sm:space-y-6 overflow-y-auto flex-1">
          ...
          <div className="flex flex-col items-center">
            <div className="relative group">
              <img
                ...
                alt="Avatar"
                className="w-20 h-20 sm:w-24 sm:h-24 rounded-full object-cover ring-4 ring-rose-400/40 shadow-lg bg-slate-50"
              />
              <button
                type="button"
          ...
          <div className="bg-slate-50 p-3 sm:p-3.5 rounded-2xl border border-slate-100 space-y-2.5 sm:space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-[11px] sm:text-xs font-black uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
                <Smile className="w-3.5 h-3.5 text-rose-500" /> Icono / Avatar
              </span>
            ...
            {avatarTab === 'presets' ? (
              <div className="grid grid-cols-4 sm:grid-cols-5 gap-2 pt-1">
                {PRESET_AVATARS.map((p) => {
                  ...
                  return (
                    ...
                    >
                      <img src={p.url} alt={p.label} className="w-9 h-9 sm:w-10 sm:h-10 rounded-full" />
                      <span className="text-[9px] sm:text-[10px] font-semibold text-slate-600 truncate w-full text-center">{p.label}</span>
                      {isSelected && (
              ...
              <div className="flex items-center gap-2 pt-1">
                <input
                  ...
                  className="flex-1 px-3 py-2 text-base sm:text-xs bg-white rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-rose-500/20"
                />
                <button
                  type="button"
                  onClick={() => handleGenerateDicebear(seedText)}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold transition cursor-pointer shadow-xs shrink-0"
                >
          ...
          {/* Name / Nickname */}
          <div>
            <label className="block text-xs font-black text-slate-700 uppercase tracking-wider mb-1.5 sm:mb-2">
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 sm:py-3 text-base sm:text-sm bg-slate-50 rounded-2xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:bg-white transition"
            />
          </div>
          ...
          <div>
              ...
              {SUGGESTED_ROLES.map((r) => {
                ...
                return (
                  <button
                    ...
                    className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer ${
                      isSelected
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-3.5 py-2 text-base sm:text-xs bg-slate-50 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:bg-white transition"
              />
            </div>
          </div>
